...
namespace AssisTec
{
    public class LancamentoFinanceiro
    {
        ...
        public void SalvarEntrada()
        {
            if (tipo == 1)
            {
                if (status == "PAGA" && string.IsNullOrWhiteSpace(dataPagamento.Replace("/","").Trim()))
                {
        ...
        public LancamentoFinanceiro carregarContaPagar(int id)
        {
            try
            {
                con.OpenConnection();
                ...
                if (rs.Read())
                {
                    this.id_lancamento = rs.GetInt32("id_conta_pagar");
                    this.descricao     = rs.GetString("descricao");
                    this.valor         = rs.GetDecimal("valor");
            ...
            {
                MessageBox.Show("Erro ao carregar conta a pagar!" + ex, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return null;
            }
        }
        ...
        public DataTable atualizarContasPagar()
        {
            ...
            {
                MessageBox.Show("Erro ao carregar contas a pagar \n" + ex.Message);
            }
        ...
        public void SalvarSaida()
        {
            if (tipo == 2)
            {
                ...
                if (Convert.ToDateTime(dataVencimento) < DateTime.Now)
                {
                    MessageBox.Show("A data de vencimento não pode ser uma data passada.", 
                        "Atenção!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
        ...
        public bool editarContaPagar(LancamentoFinanceiro lancamento)
        {
            try
            {
                con.OpenConnection();
            ...
            {
                MessageBox.Show("Erro ao editar conta a pagar \n" + ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }

        public bool registrarPagamentoSaida(int id, int forma_pagamento, string data_pagamento, DataGridView dgv)
        {
            try
            {
                con.OpenConnection();
        ...
        public bool deletarContaPagar(int idConta)
        {
            try
            {
                con.OpenConnection();
                
                if (MessageBox.Show("Deseja realmente excluir esta conta?", "Confirmar", 
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    string sqlDelete = "DELETE FROM contas_pagar WHERE id_conta_pagar = @id";
                    MySqlCommand cmdDelete = new MySqlCommand(sqlDelete, con.con);
                    cmdDelete.Parameters.AddWithValue("@id", idConta);
        ...
        public void alterarParaAtrasadoPagar()
        {
            try
            {
                con.OpenConnection();
        ...
        public (decimal totalGeral, decimal totalPago, decimal totalPendente, decimal totalAtrasado) AtualizarTotaisPagar()
        {
            try
            {
                con.OpenConnection();
                ...
                sql = @"SELECT 
                    ...
                    SUM(CASE WHEN cp.status = 'Paga' THEN cp.valor ELSE 0 END) AS total_pago,
                    SUM(CASE WHEN cp.status = 'Pendente' THEN cp.valor ELSE 0 END) AS total_pendente,
                    SUM(CASE WHEN cp.status = 'Atrasado' THEN cp.valor ELSE 0 END) AS total_atrasado
                FROM contas_pagar cp
                WHERE 1=1";

                if (temDataInicio)     sql += " AND cp.data_vencimento >= @DataInicio";
                if (temDataFim)        sql += " AND cp.data_vencimento <= @DataFim";
                if (temDescricao)      sql += " AND cp.descricao LIKE @Descricao";
                if (temStatus)         sql += " AND cp.status = @Status";
                if (temFormaPagamento) sql += " AND cp.id_forma_pagamento_fk = @FormaPagamento";
                ...
                {
                    decimal totalGeral    = reader.IsDBNull(0) ? 0 : reader.GetDecimal(0);
                    decimal totalPago     = reader.IsDBNull(1) ? 0 : reader.GetDecimal(1);
                    decimal totalPendente = reader.IsDBNull(2) ? 0 : reader.GetDecimal(2);
                    decimal totalAtrasado = reader.IsDBNull(3) ? 0 : reader.GetDecimal(3);

                    reader.Close();
                    return (totalGeral, totalPago, totalPendente, totalAtrasado);
                }
        ...
        public DataTable FiltrarContasPagar()
        {
            DataTable dt = new DataTable();
        ...
        public void GerarRelatorioContasPagarPDF(DataTable dados, decimal totalGeral, decimal totalPago, decimal totalPendente, decimal totalAtrasado)
        {
            try
            {
                con.OpenConnection();
                ...
                string caminho = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop),
                                              "Relatorio_ContasPagar_" + DateTime.Now.ToString("ddMMyyyy_HHmm") + ".pdf");
                ...
                BaseColor corLinha1   = new BaseColor(245, 248, 250);
                BaseColor corLinha2   = BaseColor.WHITE;
                BaseColor corPago     = new BaseColor(39, 174, 96);
                BaseColor corPendente = new BaseColor(230, 126, 34);
                BaseColor corAtrasado = new BaseColor(192, 57, 43);
                BaseColor corTotal    = new BaseColor(52, 73, 94);
                ...
                textoCell.Padding = 12;
                textoCell.AddElement(new Paragraph("ASSISTEC", fonteAssistec) { SpacingAfter = 2 });
                textoCell.AddElement(new Paragraph("Relatório de Contas a Pagar", fonteSubtitulo));
                ...
                AddCard("Total a Pagar", totalGeral, corTotal);
                AddCard("Pago", totalPago, corPago);
                AddCard("Pendente", totalPendente, corPendente);
                AddCard("Atrasado", totalAtrasado, corAtrasado);
                ...
                // Tabela de dados
                PdfPTable tabela = new PdfPTable(new float[] { 3, 2, 2, 2, 2, 2, 3 });
                tabela.WidthPercentage = 100;
        ...
        public void GerarReciboContaPagarPDF(int idConta)
        {
            DataRow conta = null;
            ...
            try
            {
                ...
                LEFT JOIN forma_pagamento fp
                    ON cp.id_forma_pagamento_fk = fp.id_forma_pagamento
                WHERE cp.id_conta_pagar = @idConta";
            ...
            try
            {
                ...
                string caminho = Path.Combine(
                    pastaRecibos,
                    "Recibo_Pagar_" +
                    conta["id_conta_pagar"].ToString() + "_" +
                    DateTime.Now.ToString("ddMMyyyy_HHmm") +
                    ".pdf"
                );
                ...
                Font fonteLabel       = new Font(Font.FontFamily.HELVETICA,  8, Font.BOLD,   new BaseColor(100, 100, 100));
                Font fonteDado        = new Font(Font.FontFamily.HELVETICA,  9, Font.NORMAL, new BaseColor(30, 30, 30));
                Font fonteValorGrande = new Font(Font.FontFamily.HELVETICA, 22, Font.BOLD,   new BaseColor(192, 57, 43));
                Font fonteRodape      = new Font(Font.FontFamily.HELVETICA,  7, Font.ITALIC, new BaseColor(150, 150, 150));
                Font fonteNumRecibo   = new Font(Font.FontFamily.HELVETICA,  8, Font.NORMAL, new BaseColor(180, 180, 180));
                ...
                textoCell.Padding = 10;
                textoCell.AddElement(new Paragraph("ASSISTEC", fonteEmpresa) { SpacingAfter = 2 });
                textoCell.AddElement(new Paragraph("Comprovante de Pagamento", fonteSubtitulo));
                headerTable.AddCell(textoCell);
                doc.Add(headerTable);
                ...
                PdfPCell cellNumero = new PdfPCell(new Phrase(
                    "Comprovante Nº " + conta["id_conta_pagar"].ToString().PadLeft(6, '0'),
                    fonteNumRecibo));
                cellNumero.Border = Rectangle.NO_BORDER;
                cellNumero.HorizontalAlignment = Element.ALIGN_LEFT;
                cellNumero.Padding = 4;
                ...
                cardValor.Padding = 12f;
                cardValor.HorizontalAlignment = Element.ALIGN_CENTER;
                cardValor.AddElement(new Paragraph("VALOR PAGO",
                    new Font(Font.FontFamily.HELVETICA, 8, Font.BOLD, new BaseColor(100, 100, 100)))
                    { Alignment = Element.ALIGN_CENTER });
                cardValor.AddElement(new Paragraph($"R$ {valorConta:N2}", fonteValorGrande)
                    { Alignment = Element.ALIGN_CENTER, SpacingBefore = 4 });
                ...
                // Assinatura
                PdfPTable tblAssinatura = new PdfPTable(1);
                tblAssinatura.WidthPercentage = 100;
                tblAssinatura.SpacingBefore = 20f;

                PdfPCell cellAssinatura = new PdfPCell();
                cellAssinatura.Border = Rectangle.NO_BORDER;
                cellAssinatura.Padding = 4;
                cellAssinatura.AddElement(new Paragraph("_______________________________",
                    new Font(Font.FontFamily.HELVETICA, 9, Font.NORMAL, new BaseColor(150, 150, 150))));
                cellAssinatura.AddElement(new Paragraph("Assinatura do Responsável",
                    new Font(Font.FontFamily.HELVETICA, 7, Font.NORMAL, new BaseColor(130, 130, 130)))
                    { Alignment = Element.ALIGN_CENTER });
                tblAssinatura.AddCell(cellAssinatura);
                doc.Add(tblAssinatura);
                ...
                MessageBox.Show("Comprovante gerado com sucesso!", "Sucesso",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
