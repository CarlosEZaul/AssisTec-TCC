...
{
    public static class GeradorPdfUsuario
    {
        public static void GerarRelatorioGeral(UsuarioDTO.UsuariosRelatorioDTO dados, string caminhoDestino)
        {
            ...
            try
            {
                using (FileStream fs = new FileStream(caminhoDestino, FileMode.Create))
                {
                    ...
                    PdfPTable headerTable = CriarCabecalho("Relatório de Controle de Usuários", fontTitulo, fontSubtitulo, fontMeta);
                    doc.Add(headerTable);
            ...
            {
                throw new Exception("Erro ao gerar o relatório geral de usuários: " + ex.Message, ex);
            }
        }

        public static void GerarRelatorioIndividual(UsuarioDTO.UsuarioComOrdemServicoDTO dados, string caminhoDestino)
        {
            Document doc = new Document(PageSize.A4, 36, 36, 36, 36);
            ...
            try
            {
                using (FileStream fs = new FileStream(caminhoDestino, FileMode.Create))
                {
                    ...
                    PdfPTable headerTable = CriarCabecalho("Relatório de Produtividade do Usuário", fontTitulo, fontSubtitulo, fontMeta);
                    doc.Add(headerTable);
                    ...
                    foreach (var item in dados.Ordens)
                    {
                        ...
                        bool finalizada = item.Status != null && (item.Status.Equals("Finalizado", StringComparison.OrdinalIgnoreCase) || item.Status.Equals("Entregue", StringComparison.OrdinalIgnoreCase) || item.Status.Equals("FINALIZADA", StringComparison.OrdinalIgnoreCase));
            ...
            {
                throw new Exception("Erro ao gerar relatório individual do usuário: " + ex.Message, ex);
            }
        }
        ...
        private static string ObterDescricaoNivel(int nivel)
        {
            switch (nivel)
            {
