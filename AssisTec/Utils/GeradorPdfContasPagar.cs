...
{
    public static class GeradorPdfContasPagar
    {
        public static void GerarRelatorioGeral(ContasPagarDto.ContasPagarRelatorioDTO dados, string caminhoDestino)
        {
            ...
            try
            {
                using (FileStream fs = new FileStream(caminhoDestino, FileMode.Create))
                {
                    ...
                    PdfPTable headerTable = CriarCabecalho("Relatório Geral de Contas a Pagar", fontTitulo, fontSubtitulo, fontMeta);
                    doc.Add(headerTable);
                    ...
                    summaryTable.AddCell(CriarCardResumo("TOTAL PAGO", dados.TotalPagar.ToString("C2"), new BaseColor(56, 161, 105), fontMeta, fontTitulo));
                    ...
                    dataTable.SetWidths(new float[] { 6f, 28f, 13f, 11f, 11f, 11f, 10f, 10f });
                    dataTable.SpacingBefore = 5f;

                    string[] headers = { "ID", "Descrição", "Valor", "Emissão", "Vencimento", "Pagamento", "Status", "Forma" };
                    ...
                    if (dados.Itens != null)
                    {
                        ...
                        {
                            dataTable.AddCell(new PdfPCell(new Phrase(item.IdContaPagar.ToString(), fontRegular)) { Padding = 6, BorderColor = new BaseColor(226, 232, 240) });
                            ...
                            if (statusStr.Equals("PAGA", StringComparison.OrdinalIgnoreCase) || statusStr.Equals("PAGO", StringComparison.OrdinalIgnoreCase))
                            {
                            ...
                            dataTable.AddCell(new PdfPCell(new Phrase(item.FormaPagamentoDescricao ?? "-", fontRegular)) { Padding = 6, HorizontalAlignment = Element.ALIGN_CENTER, BorderColor = new BaseColor(226, 232, 240) });
                        }
                    }
            ...
            {
                throw new Exception("Erro ao gerar o relatório geral de contas a pagar: " + ex.Message, ex);
            }
        }

        public static void GerarRelatorioIndividual(ContasPagarDto dto, string caminhoDestino)
        {
            ...
            try
            {
                using (FileStream fs = new FileStream(caminhoDestino, FileMode.Create))
                {
                    ...
                    PdfPTable headerTable = CriarCabecalho("Comprovante / Detalhes da Conta a Pagar", fontTitulo, fontSubtitulo, fontMeta);
                    doc.Add(headerTable);
                    ...
                    string[,] infoCampos = {
                        { "Código:", dto.IdContaPagar.ToString(), "Valor:", dto.Valor.ToString("C2") },
            ...
            {
                throw new Exception("Erro ao gerar relatório individual da conta a pagar: " + ex.Message, ex);
            }
        }
