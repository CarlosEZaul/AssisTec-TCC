...
namespace AssisTec.Service
{
    public class MovimentacaoEstoqueService
    {
        ...
        public void GerarRelatorioPdf(DateTime? dataInicio, DateTime? dataFim, string produtoSelecionado, string tipoMovimentacao, string caminhoDestino)
        {
            try
            {
                var dadosFiltrados = _movimentacaoEstoqueRepository.Filtrar(dataInicio, dataFim, produtoSelecionado, tipoMovimentacao) as List<dynamic>;

                if (dadosFiltrados == null)
                {
                    var listaGenerica = _movimentacaoEstoqueRepository.Filtrar(dataInicio, dataFim, produtoSelecionado, tipoMovimentacao) as IEnumerable;
        ...
        private void ExecutarGeracaoPdf(MovimentacoesEstoqueDTO dados, string caminhoDestino)
        {
            ...
            summaryTable.AddCell(CriarCardResumo("TOTAL ENTRADAS (QTD)", dados.TotalEntradas.ToString(), new BaseColor(56, 161, 105), fontMeta, fontTitulo));
            summaryTable.AddCell(CriarCardResumo("TOTAL SAÍDAS (QTD)", dados.TotalSaidas.ToString(), new BaseColor(229, 62, 98), fontMeta, fontTitulo));
            summaryTable.AddCell(CriarCardResumo("BALANÇO LÍQUIDO", (dados.BalancoLiquido >= 0 ? "+" : "") + dados.BalancoLiquido, new BaseColor(49, 130, 206), fontMeta, fontTitulo));
            ...
            dataTable.SetWidths(new float[] { 8f, 25f, 12f, 10f, 13f, 15f, 17f });
            dataTable.SpacingBefore = 5f;

            string[] headers = { "ID", "Produto", "Data", "Qtd.", "Valor", "Tipo", "Descrição" };
            ...
            {
                dataTable.AddCell(new PdfPCell(new Phrase(item.IdMovimentacao.ToString(), fontRegular)) { Padding = 6, BorderColor = new BaseColor(226, 232, 240) });
                dataTable.AddCell(new PdfPCell(new Phrase(item.Produto, fontRegular)) { Padding = 6, BorderColor = new BaseColor(226, 232, 240) });
                dataTable.AddCell(new PdfPCell(new Phrase(item.Data.ToString("dd/MM/yyyy"), fontRegular)) { Padding = 6, HorizontalAlignment = Element.ALIGN_CENTER, BorderColor = new BaseColor(226, 232, 240) });
                ...
                bool isEntrada = item.TipoMovimentacao.ToLower().Contains("entrada");
                PdfPCell typeCell = new PdfPCell(new Phrase(item.TipoMovimentacao, fontBold));
